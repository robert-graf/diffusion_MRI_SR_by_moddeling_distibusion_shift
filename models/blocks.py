...
class ResBlockConfig:  # TODO REMOVE CONFIG
    ...
    two_cond: EmbeddingType | None = None
...
class ResBlock(TimestepBlock):
    ...
    def __init__(self, conf: ResBlockConfig):
        ...
        if conf.use_condition:
            ...
            if conf.two_cond is None:
                pass
            elif conf.two_cond == EmbeddingType.adaptivenonzero or conf.two_cond == EmbeddingType.adaptivenonzero.name:
                self.cond_emb_layers = nn.Sequential(nn.SiLU(), linear(conf.cond_emb_channels, conf.out_channels))
            elif conf.two_cond == EmbeddingType.ConvEmb:
                ...
                raise NotImplementedError(conf.two_cond)
            #############################
    ...
    def _forward(self, x, emb=None, cond=None, lateral=None):
        ...
        if self.conf.use_condition:
            ...
            if self.conf.two_cond is not None:
...
def apply_conditions(
    ...
    for i, each in enumerate(scale_shifts_):
    ...
    pre_layers, mid_layers, post_layers = layers[0], layers[1:-2], layers[-2:]
