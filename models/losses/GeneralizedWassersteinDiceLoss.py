# https://github.com/LucasFidon/GeneralizedWassersteinDiceLoss/blob/master/generalized_wasserstein_dice_loss/loss.py
import numpy as np
import torch
...
class GeneralizedWassersteinDiceLoss(_Loss):
    ...
    def __init__(self, dist_matrix, weighting_mode="default", reduction="mean"):
        ...
        assert weighting_mode in SUPPORTED_WEIGHTING, "weighting_mode must be in %s" % str(SUPPORTED_WEIGHTING)

        self.M = dist_matrix
        if isinstance(self.M, np.ndarray):
            self.M = torch.from_numpy(self.M)
        if torch.max(self.M) != 1:
            print("Normalize the maximum of the distance matrix " "used in the Generalized Wasserstein Dice Loss to 1.")
    ...
    def compute_generalized_true_positive(self, alpha, flat_target, wasserstein_distance_map):
        ...
        flat_target_extended = torch.unsqueeze(flat_target, dim=1)  # b,s -> b,1,s
        alpha_extended = torch.gather(alpha_extended, index=flat_target_extended, dim=1)  # b,C,s -> b,1,s
    ...
    def compute_alpha_generalized_true_positives(self, flat_target):
        ...
        if self.alpha_mode == "GDL":  # GDL style
            ...
            one_hot = (
                F.one_hot(  # shape: b,c,s
                    flat_target, num_classes=self.num_classes
                )
                .permute(0, 2, 1)
                .float()
            )
