...
class Unet(nn.Module):
    def __init__(
        ...
        attention_layer: None | list[int] = None,
    ):
        super().__init__()
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            do_att = attention_layer is None or ind in attention_layer
            if do_att:
                print("Attention lvl", ind, dim_in, dim_out)
            att = Residual(PreNorm(dim_out, LinearAttention(dim_out))) if do_att else nn.Identity()
