...
def hessian_penalty(G, z, k=2, epsilon=0.1, reduction=torch.max, return_separately=False, G_z=None, **G_kwargs) -> torch.Tensor:
    """
...
def multi_stack_var_and_reduce(sdds, reduction=torch.max, return_separately=False) -> torch.Tensor:
    ...
    return sum_of_penalties  # type: ignore
