...
class Dataset_CSV_super(Dataset_CSV):
    ...
    def get_img(self, index, c=None):
        ...
        crop = -(patch_hr.shape[-2] % sli)
        if crop != 0:
            patch_hr = patch_hr[..., :crop, :]
            patch_lr = patch_lr[..., :crop, :]
        assert patch_hr.shape[-2] % 6 == 0, (patch_hr.shape, crop)
        ...
        a1 = (
            patch_lr[..., 4::6, :].clone() * r1
            + patch_lr[..., 1::6, :].clone() * r2
            + patch_lr[..., 2::6, :].clone() * r3
            + patch_lr[..., 3::6, :].clone() * r4
        ) / (r1 + r2 + r3 + r4)
        patch_lr = torch.stack([a1 for _ in range(6)], -2).reshape(patch_hr.shape)
        ...
        out = {"img": patch_hr, "img_lr": patch_lr}
