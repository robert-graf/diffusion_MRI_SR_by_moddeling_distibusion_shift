...
WEIGHTS_URL_ = "https://github.com/robert-graf/diffusion_MRI_SR_by_moddeling_distibusion_shift/releases/download/v1.0.0/"
...
def get_weights_dir(idx) -> Path:
    if env_name in os.environ:
        ...
        assert Path(__file__).parent.name == "scripts"

        weights_dir = Path(__file__).parent.parent / "lightning_logs_dae/"

    weights_dir.parent.mkdir(exist_ok=True)
    weights_dir.mkdir(exist_ok=True)
    weights_dir = weights_dir / idx
...
def download_weights(idx: str) -> Path:
    weights_dir = get_weights_dir(idx)
